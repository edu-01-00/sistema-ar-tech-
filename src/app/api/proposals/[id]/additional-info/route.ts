...
import { updateProposalAdditionalInfoSchema } from "@/lib/validations/proposal";
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = parseBody(updateProposalAdditionalInfoSchema, await request.json());
    ...
    await prisma.proposal.update({ where: { id: proposal.id }, data: { additionalInfo: data.additionalInfo || null } });
    ...
    await writeAuditLog({
      ...
      description: `Informações adicionais da proposta ${proposal.code} atualizadas.`,
    });
