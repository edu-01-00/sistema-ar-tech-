...
// Qualidade do Ar. Upload persistente, com substituição/remoção.
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  try {
    await requirePermission("collection_points.view");
    const detail = await prisma.collectionPointAirQuality.findUnique({ where: { collectionPointId: params.id } });
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const point = await prisma.collectionPoint.findUnique({ where: { id: params.id }, include: { airQualityDetail: true } });
    if (!point) throw new ApiError("Ponto de coleta não encontrado.", 404);
    if (point.matrix !== "QUALIDADE_AR") throw new ApiError("Este ponto não é da matriz Qualidade do Ar.", 422);
    ...
    const previousKey = point.airQualityDetail?.imageStorageKey ?? null;

    await prisma.collectionPointAirQuality.upsert({
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.collectionPointAirQuality.update({
      where: { collectionPointId: params.id },
